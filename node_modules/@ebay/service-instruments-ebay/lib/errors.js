...
const logger = require('@ebay/logging-inc').logger('service-instruments-ebay/errors');
/**
 *  Params:
...
const ErrorCategory = module.exports.ErrorCategory = {
    APPLICATION: 'APPLICATION',
    USER: 'USER',
    SYSTEM: 'SYSTEM'
};

const RuntimeError = module.exports.RuntimeError = function RuntimeError() {
    const args = [].slice.call(arguments);
    const options = args.reduce((memo, param) => {
        updateOptions(param, memo);
        return memo;
    }, {});

    let msg = options.message || 'RuntimeError';
    ...
    this.type = options.error && options.error.type &&
        options.error.type !== 'error' ? options.error.type : options.type || 'error';
