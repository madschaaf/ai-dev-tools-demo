...
const logger = require('@ebay/logging-inc').logger('service-instruments-ebay/resolver');

const customInstruments = {};
let instrumentsMetadata = {};
let configValue;
let modConfig;
...
function resolveInstrumentMeta(name) {
    ...
    if (typeof name === 'object') {
        ...
        const location = Utils.tryResolve(meta.module);
        if (!location) {
...
function updateInstrumentsMetadata(instrumentsMeta) {
    ...
    Object.keys(instrumentsMeta).forEach((name) => {
        const meta = instrumentsMeta[name];
        meta.name = name;

        const instrument = tryLocal(meta.module) ||
...
function tryLocal(name) {
    const root = 'service-instruments-ebay/';
    const pos = name.indexOf(root);
    if (pos !== -1) {
...
module.exports.isReady = new Promise(resolve => {
    loadInstrumentsMetadata(resolve);
});

module.exports.reset = function reset(clearCache) {
    configValue = undefined;
    if (clearCache) {
        instrumentsMetadata = {};
    }
