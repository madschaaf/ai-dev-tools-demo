...
const ATTRIB_ACCESSOR = 'attributes';

const SOA_HEADERS = require('./soa-headers');
const SUCURE_HEADERS = _.reduce([
...
module.exports.instrument = function instrument(wsdl, options) {
    if (options.namespaces) {
        ...
        if (options.namespaces.list) {
            for (const key in options.namespaces.list) {        //eslint-disable-line
                const opt = options.namespaces.list[key];
                if (opt.byParent) {
...
function objectToXML(obj, name, namespace, xmlns, ctx) {
    ...
    if (!/^\$|^\#/.test(name)) {
        if (first) {
            ...
            const methodDef = this.definitions.messages[name];
            ctx = {};
            ctx[name] = methodDef.parts;
        }
    }

    const nsEnabled = namespace !== '';
    if (nsEnabled) {
    ...
    let ns = namespace && nsEnabled ? `${namespace}:` : '';

    if (Array.isArray(obj)) {
        for (let i = 0, item; item = obj[i]; i++) {     //eslint-disable-line
        ...
        if (obj && typeof obj === 'object') {
            const nextCtx = ctx[name] || ctx[`${name}[]`];
