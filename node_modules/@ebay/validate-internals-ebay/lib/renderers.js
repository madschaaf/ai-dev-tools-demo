...
const _ = require('underscore'),
    ...
    Httpunch = require('@ebay/httpunch'),
    ...
    commonModel = function commonModel(forceXml, clientIpAddress) {
        ...
        return {
            ...
            'cal': util.format('http://appmon.vip.%sebay.com/logviewui/environment/%s/pool/%s/machine/%s/machineOverview',
                ...
                appContext.calPoolName || (appContext.poolName + (env.isQA() && appContext.odbMoName &&
                    appContext.paasRealm === 'staging' && `-${appContext.odbMoName}` || '' )),
                os.hostname()),
            'clientIpAddress': clientIpAddress || ip.address(),
            'forceXml': forceXml,
            'envoyEnabled': ViUtils.isEnvoyEnabled() || false,
            'containerPdlcEnabled': ViUtils.isContainerPdlcEnabled() || false,
            'classOfService': process.env.CLASS_OF_SERVICE || 'local'
        };
    };
...
if (cachedIpAddress === null) {
    cachedIpAddress = utilsEbay.getIpAddress() || '127.0.0.1';
}
...
module.exports.buildServiceComponentsTree = function buildServiceComponentsTree(statusQuery) {
    ...
    return beanList.reduce((components, component) => {
        ...
        const protocol = theBean.get('protocol') ? theBean.get('protocol') : 'http:';
        const endpoint = `${protocol}//${theBean.get('hostname')}${theBean.get('port') ? `:${theBean.get('port')}` : ''}`;
        const port = theBean.get('port') ? `:${theBean.get('port')}` : '';
        const origendpoint = `${theBean.get('_orig_protocol') ? theBean.get('_orig_protocol'):protocol}//${theBean.get('_orig_hostname') ? theBean.get('_orig_hostname'):theBean.get('hostname')}${theBean.get('_orig_port') ? `:${theBean.get('_orig_port')}` : `${port}`}`;
        const urn = theBean.get('urn') || '';

        //tls.DEFAULT_MIN_VERSION is available only for node version >=11.x
        const tlsVersion = protocol === 'https:' ? (theBean.get('tlsSocketProtocol') || theBean.get('secureProtocol') || '') : '';
...
module.exports.renderServiceComponentsTree = function renderServiceComponentsTree(req, res) {
    ...
    try {
        ...
        var model = _.extend(commonModel(req.query.forceXml, utilsEbay.getRemoteAddr(req)), {
...
module.exports.buildTroubleshootingModel = function buildTroubleshootingModel(callback) {
    ...
    const modifiedAttributes = modifiedBeans.reduce((memo, bean) => {
        ...
        modifiedAttrs.forEach(attr => {
            ...
            if (attr.isNew) {
                if (attr.value === '') {
                    newValue = attr.defaultValue;
                    oldValue = attr.origValue !== undefined ? attr.origValue : '';
                } else {
                    newValue = attr.value;
                    oldValue = attr.defaultValue;
                }
            } else {
                newValue = attr.value || attr.origValue && attr.defaultValue;
                oldValue = attr.origValue || attr.defaultValue;
            }
    ...
    collectWorkerInfo((err, wksHostInfo) => {
        ...
        const srePerfTrends = `http://sre.vip.ebay.com/ex/c/perfmontrend?trendType=perfmon&poolName=${appContext.calPoolName || appContext.poolName}&fromDate=${fromDate}&toDate=${toDate}&dtOverride=&tsd=on`;
...
module.exports.buildEnvoyInfoModel = function buildEnvoyInfoModel(callback) {
    let envoyDashboard, envoyAdminAccessLogs, envoyStdOutErrLogs;
    //for prod and preProd : console.sherlock.io, all others : console.dev.sherlock.io
    const envoyDashboardRootUrl = (env.isNotPreProd() && env.isNotProd()) ? 'console.dev.sherlock.io' : 'console.sherlock.io';
    if(ViUtils.isMeshEnabled()) {
        envoyDashboard = `https://${envoyDashboardRootUrl}/d/IS_xJEvpl/istio-sidecar-dashboard?var-application=${appContext.appName}&var-pod=${os.hostname()}`;

        const envoyLogLink = process.env.ENVOY_LOG_LINK || '';
        envoyAdminAccessLogs = util.format(envoyLogLink, 'admin_access');
        envoyStdOutErrLogs = util.format(envoyLogLink, 'stdout|stderr');
    } else {
        envoyDashboard = `https://${envoyDashboardRootUrl}/d/RSEBuH-iz/envoy-dashboard?orgId=1&var-application=${appContext.appName}&var-node=${os.hostname()}`;
        if(ViUtils.isContainerPdlcEnabled()) {
            envoyDashboard = `https://${envoyDashboardRootUrl}/d/__h3UzYGz/envoy-dashboard-for-container-pdlc?orgId=1&var-application=r1${appContext.nuggetName}&var-node=${os.hostname()}`;
        }
    }

    const quickLinks = {
        envoyLogs: './logs',
        envoyLoggingConfigBean: '/admin/v3console/ViewConfigCategoryXml?id=nodejs.config.EnvoyLoggingConfigBean',
        envoyDashboard: envoyDashboard,
        envoyAdminAccessLogs: envoyAdminAccessLogs,
        envoyStdOutErrLogs: envoyStdOutErrLogs,
        isMeshEnabled: ViUtils.isMeshEnabled()
    };

    Async.parallel({
            envoyHealthStatus: (cb) => {
                componentStatus.getStatus('EnvoyHealthComponentStatus', (healthInfo) => {
                    cb(null, getAttributes(healthInfo));
                });
            },

            envoyServerInfo: (cb) => {
                componentStatus.getStatus('EnvoyServerInfoComponentStatus', (envoyServerInfo) => {
                    cb(null, getAttributes(envoyServerInfo));
                });
            },

            beatsInfo: (cb) => {
                componentStatus.getStatus('BeatsPackageComponentStatus', (beatsInfo) => {
                    cb(null, getAttributes(beatsInfo));
                });
            }
        },
        (err, result) => {
            callback(null, {
                envoyHealthStatus: result.envoyHealthStatus,
                envoyServerInfo: result.envoyServerInfo,
                quickLinks: quickLinks,
                beatsInfo: result.beatsInfo
            });
        });
};
...
module.exports.renderTroubleshooting = function renderServiceComponentsTree(req, res) {
    createSafeFunction(() => module.exports.buildTroubleshootingModel(createSafeFunction((err, model) => {
        model = _.extend(
            commonModel(req.query.forceXml, utilsEbay.getRemoteAddr(req)),
            model);
...
module.exports.renderEnvoyInfo = function renderEnvoyStatusTree(req, res) {
    if (!ViUtils.isEnvoyEnabled()) {
        res.status(404).send(`Envoy Not Enabled for ${appContext.poolName} in ${process.env.NODE_ENV} environment`);
        return;
    }

    createSafeFunction(() => module.exports.buildEnvoyInfoModel(createSafeFunction((err, model) => {
        model = _.extend(
            commonModel(req.query.forceXml, utilsEbay.getRemoteAddr(req)),
            model);

        renderXmlResult('envoy', model, req, res);
    })))();
};
...
module.exports.renderConfigBean = function renderConfigBean(req, res) {
    ...
    if (!bean) {
        ...
        for (var i = 0; i < attribs.length; i++) {
            attribs[i].value = attribs[i].value === '' ? attribs[i].defaultValue : attribs[i].value;
            ...
            if (attribs[i].type === 'timestamp') {
                attribs[i].value = moment(attribs[i].value).format('ddd MMMM DD HH:mm:ss Z YYYY');
            }
            if (attribs[i].type === 'object') {
                attribs[i].value = JSON.stringify(attribs[i].value);
            }
        }
        var model = _.extend(commonModel(req.query.forceXml, utilsEbay.getRemoteAddr(req)), {
...
// get fire marshal policy function
function renderFireMarshalPolicy(req, res){
    const component = req.query.component;
    const statusModel = componentStatus.getStatus(component);
    return buildAndRender(statusModel, 'fire-marshal-policy-status', req, res);
}

module.exports.renderFireMarshalPolicy = renderFireMarshalPolicy;

//get cookie audit function
const CookieAuditInstrumentation = require('@ebay/cookies-ebay').CookieAuditInstrumentation;
const cookieAuditInstrumentation = new CookieAuditInstrumentation();

function renderCookieCookieletComponent(template, req, res) {
    const component = req.query.component;
    const statusModel = componentStatus.getStatus(component);

    const componentProperties =
        component === 'CookieStats' ? Array.from(cookieAuditInstrumentation.getTotalCookieStats().values())
            : Array.from(cookieAuditInstrumentation.getTotalCookieletStats().values());

    componentProperties.sort((a, b) => a.id - b.id);

    statusModel.properties = componentProperties;
    return buildAndRender(statusModel, template, req, res);
}

module.exports.renderCookieCookieletComponent = renderCookieCookieletComponent;


function renderComponentStatusByTemplate(template, req, res) {
    ...
    if (componentStatus.isStatusAsync(component)) {
        componentStatus.getStatus(component, (statusModel) => {
            buildAndRender(statusModel, template, req, res);
        });
    } else {
...
module.exports.renderComponentStatusByTemplate = renderComponentStatusByTemplate;

module.exports.renderComponentStatus = function renderComponentStatus(req, res) {
    renderComponentStatusByTemplate('component-status', req, res);
};
