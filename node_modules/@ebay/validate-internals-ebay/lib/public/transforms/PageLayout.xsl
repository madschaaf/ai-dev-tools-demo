...
<xsl:param name='custom-css'/>
    ...
    <script language="JavaScript">
      ...
      <body id="vi_page">
        <xsl:if test="$root-node/@envoyEnabled = 'true'"> 
          <span style="float: right; padding-top: 20px"> <a href="ValidateInternals?component=Envoy%20Info"><div class="envoy_logo_image"></div></a></span>
        </xsl:if>

        <table id="logoTable">
          <tr>
            <xsl:choose>
              <xsl:when test="$root-node/@containerPdlcEnabled = 'true'">
                <td valign="middle" align="left" rowspan="2"><a href="ValidateInternals"><div class="logo_image_container_pdlc"/></a></td>
              </xsl:when>
              <xsl:otherwise>
                <td valign="middle" align="left" rowspan="2"><a href="ValidateInternals"><div class="logo_image"/></a></td>
              </xsl:otherwise>
            </xsl:choose>
            <td valign="middle" align="left" colspan="3"><a href="ValidateInternals"><div class="logo_admin"></div></a></td>
          </tr>
          <tr>
           <td class="space9h">&#160;</td>
           <td>
        ...
        <xsl:if test="$root-node/@classOfService">
          <xsl:choose>
            <xsl:when test="$root-node/@classOfService = 'production'">
              <div id="production" class="class_of_service_wrapper" align="center">
                Class of Service is <xsl:value-of select="$root-node/@classOfService"/>
              </div>
            </xsl:when>
            <xsl:when test="$root-node/@classOfService = 'pre-production'">
              <div id="pre-production" class="class_of_service_wrapper" align="center">
                Class of Service is <xsl:value-of select="$root-node/@classOfService"/>
              </div>
            </xsl:when>
              <xsl:otherwise>
              <div id="non_production" class="class_of_service_wrapper" align="center">
                Class of Service is <xsl:value-of select="$root-node/@classOfService"/>
              </div>  
            </xsl:otherwise>
          </xsl:choose>
...
<xsl:template match="HeaderConfig">
  ...
  <li id="{$idx}" class="{@class}"><a link="none" href="{@href}" title="{@desc}"><xsl:value-of select="@desc"/></a></li>
</xsl:template>
</xsl:stylesheet>
