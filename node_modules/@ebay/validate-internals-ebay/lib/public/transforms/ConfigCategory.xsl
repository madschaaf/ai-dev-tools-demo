...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="ConfigCategory">
        <div class="table_title" id="infoTableId_title">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="3" id="infoTableId" class="display">
            ...
            <tr>
                <td>Initializable</td>
                <td>
                    <xsl:value-of select="@initializable"/>
                </td>
            </tr>
            <tr>
                <td>Site&#160;Operations&#160;Command</td>
                <td>
                    <xsl:value-of select="@site-ops-command"/>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <td>Persistent</td>
                <td>
                    <xsl:value-of select="@persist-updates"/>
                </td>
            </tr>
            <tr>
                <td>Persistent&#160;Location</td>
                <td>
                    <xsl:value-of select="@persistent-location"/>
                </td>
            </tr>
        </table>
        ...
        <div class="legend_wrapper">
            ...
            <div class="legend_right">
                <table class="legend_table">
                    <tr>
                        <td class="updated-row-head">Current&#160;changes</td>
        ...
        <form id="formAddNewRow" action="" title="Add a new property">
            ...
            <table id="formAddNewRow_table">
                <tr>
                    <td>
                        <label for="nameAddId">Name</label>
                    </td>
                    <td>
                        <input type="text" name="name" id="nameAddId" rel="0"/>
                    </td>
                </tr>
                <tr>
                    <td>
                        <label for="valueAddId">Value</label>
                    </td>
                    <td>
                        <input type="text" name="value" id="valueAddId" rel="1"/>
                    </td>
                </tr>
            </table>
        </form>
        ...
        <tr>
            ...
            <xsl:choose>
                <xsl:when test="@updateStatus = 'success'">
                    <td class='updated-row-head-success'>
                        <xsl:value-of select="@name"/>
                    </td>
                </xsl:when>
                <xsl:when test="@updateStatus = 'failed'">
                    <td class='updated-row-head-failure'>
                        <xsl:value-of select="@name"/>
                    </td>
                </xsl:when>
                <xsl:otherwise>
            ...
            <td id="td_{$attrId}">
                <xsl:if test="@persist-policy='Always'">
                    <xsl:attribute name="title">(Persist : <xsl:value-of select="@persist-policy"/>)
                    </xsl:attribute>
                </xsl:if>
    ...
    <xsl:template match="attribute" mode="initpage">
        ...
        <xsl:choose>
            ...
            <xsl:when test="validValues">,
                type :'select',
                data :"{
                <xsl:for-each select="validValues/validValue">'<xsl:value-of select="@value"/>':'<xsl:value-of
                        select="@name"/>',
                </xsl:for-each>
                }"
            </xsl:when>
    ...
    <xsl:template match="attribute" mode="ServeTraffic">
        ...
        <tr>
            ...
            <td style="color: {$fontcolor}" id="td_{@name}">
                <xsl:value-of select="@value"/>
                <xsl:if test="@persist-policy='Always'">
                    <xsl:attribute name="title">(Persist : <xsl:value-of select="@persist-policy"/>)
                    </xsl:attribute>
