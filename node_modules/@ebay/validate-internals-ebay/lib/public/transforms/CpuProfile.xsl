  ...
  <xsl:template match="ProfileRequest[@file != '']" mode="cpu">
      ...
      <script>
          ...
          var content = ', click here to <a download=' + path +
            ' href="./logs?logfile=' + path + '">download</a><br/>';
          if (view) {
              counter = 0;
          }
      ...
      <script type="text/javascript"><![CDATA[
          ...
          function showFlames() {
              d3.json("./logs?logfile=]]><xsl:value-of select="@file" /><![CDATA[", function(error, data) {
                $('#progress').hide();
          ...
          function adjustBaseTime(profile) {
              profile.head.totalTime = profile.head.children.reduce(function reduce(memo, child) {
                  if (child.functionName === '(idle)' || child.functionName === '(program)') {
                      // skip idle time
                      child.totalTime = 5;
                  }
          ...
          function chromeProfileToCallgrind(profile) {
              ...
              // if natively (nodejs profile) generated profile data
              // then first generate fast accessor by building map of nodes
              // to make it compatible with original data structure
              if (timedProfile.nodes) {
                  // build map
                  const _nodes = timedProfile.nodes.reduce((accum, node) => {
                      if (accum[node.id]) {
                          console.error(`[ERROR] Unexpected state: node${node.id} has duplicate entry, moreInfo:${JSON.stringify(node)}`);
                      }
                      Object.assign(node, node.callFrame);
                      delete node.callFrame;
                      if (node.lineNumber !== undefined) {
                          node.lineNumber =+ 1; // as it counts from line 0, not 1
                      }
                      accum[node.id] = node;
                      return accum;
                  }, {});
                  // convert to expected tree like structure
                  function normalizeNode(node) {
                    node.children = (node.children || []).map(n => {
                        const cnode = _nodes[n];
                        return normalizeNode(cnode);
                    });
                    return node;
                  }

                  timedProfile.head = normalizeNode(timedProfile.nodes[0]);
                  timedProfile.startTime = parseInt(timedProfile.startTime/1e6);
                  timedProfile.endTime = parseInt(timedProfile.endTime/1e6);
              }
