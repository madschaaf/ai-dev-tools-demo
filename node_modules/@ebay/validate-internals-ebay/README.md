...
[![Build Status](https://go/-/nodejsci/job/nodejs/job/validate-internals-ebay/job/master/badge/icon)](https://go/-/nodejsci/job/nodejs/job/validate-internals-ebay/job/master/) [![Dependency Status](https://appvalidator.vip.qa.ebay.com/nodejs/validate-internals-ebay.svg)](https://appvalidator.vip.qa.ebay.com/nodejs/validate-internals-ebay)
[![Code Coverage](https://badgeme.vip.qa.ebay.com/badge/sonar/v0/coverage/validate-internals-ebay)](http://sonar/dashboard/index?id=validate-internals-ebay)
...
The implementation of this should be similar to the raptor one [https://github.corp.ebay.com/raptor-io-components/raptor-validateinternals-component](https://github.corp.ebay.com/raptor-io-components/raptor-validateinternals-component)

## Note for Node v8 users
Since version `5.0.0`  `v8-profiler-node8` is not included anymore. 
You should explicitly add it to your `package.json`:
...
It also displays envoy & nodeagent logs in environments where these are available. Additionally, service discovery cache file `.discovery.json` is also exposed through this.
...
ValidateInternals now exposes Prometheus metrics endpoint on `/admin/prometheus/<namespace>`.
Example:
```
 * /admin/prometheus/platform - platform metrics
 * /admin/prometheus/custom - for metrics defined as `new Metrics('custom')`
```
For more details about prometheus, refer [Node.js client(prometheus-ebay)](https://github.corp.ebay.com/nodejs/prometheus-ebay) or [prometheus.io](https://prometheus.io/)
