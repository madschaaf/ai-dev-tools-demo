...
var ConfigBeanAdaptor = module.exports.ConfigBeanAdaptor = function ConfigBeanAdaptor(configbean) {
     ...
     */
    this.fireChangeEvent = function (deletedProperties) {
        if (this.eventTimer) {
            clearTimeout(this.eventTimer);
        }
        this.eventTimer = setTimeout(function () {
            this.emit('changed', deletedProperties);
        }.bind(this), 5000);
    };
    ...
    */
    ...
    this.lastUpdated = function (name) {
        if (!name) {
            return getLastUpdated(this.configbean);
        }
        name = name.replace(/:/g, CONS.delimiter);
    ...
    */
    this.getAttributes = function getAttributes(modified) {
        ...
        attribs.forEach(function (elt) {
            ...
            elt = _.clone(elt);
            // empty value and origValue means that the attribute was not modified unless it is new
            if (elt.value === '' && !elt.isNew && modified && !elt.origValue) {
                return;
            }
            /**
             * Important: modified is used to get only changed values for troubleshooting page
             *   while modified=false we pull all beans and that requires to move default values to
             *   value so it can be displayed correctly (this is backwards compatibility with vi and the follow up change
             *   in validate internals will do fallback to defaultValue when value is empty)
             */
            if (!modified && elt.value === '') {
    ...
    this.reset = function reset() {
        ...
        this.fireChangeEvent(this.configbean.deletedProperties);
        delete this.configbean.deletedProperties;
        return this.configbean;
    };

    this.clear = function clear() {
        this.configbean.attributes = [];
        this.fireChangeEvent();
        return this.configbean;
    };

    if (configbean.persist === false && utils.isDeployRestart()) {      //persist is default true, unless explicitly marked false
        this.reset();
    }
};
...
/**
 * This method is used by validate internal to pull modified configs into troubleshooting page
 * */
module.exports.getConfigBeans = function getConfigBeans(modified) {
    ...
    return Object.keys(savedConfigBeans).reduce((memo, id) => {
        ...
        if (!modified || (bean.configbean.created < getLastUpdated(bean.configbean))) {
            memo.push(bean);
        }
        return memo;
    }, []);
...
function getLastUpdated(configbean, name) {
    ...
    if (name === undefined) {
        let lastUpdated = 0;
        for (let index = 0; index < configbean.attributes.length; index++) {
            attr = configbean.attributes[index];
            lastUpdated = Math.max(attr.lastUpdated || 0, lastUpdated);
        }
        return lastUpdated || configbean.created;
    }
    attr = findByName(configbean, name);
...
function setConfigBeanValue(configbean, name, value, persist) {
    ...
    if (persist) {
        ...
        val = setConfigBeanProp(configbean, name, 'defaultValue', value);
        if (val instanceof Error) {
            return val;
        }

        val = setConfigBeanProp(configbean, name, 'value', '');
    } else {
    ...
    delete configbean.deletedProperties;
...
function setConfigBeanProp(configbean, name, attrName, value) {
    var err;
    ...
    if (attr) {
        ...
        attr[attrName] = value;
        if (attrName === 'defaultValue' && attr.lastUpdated && oldVal !== value) {
            attr.origValue = oldVal;
        }
...
function resetToDefault(configbean) {
    ...
    configbean.attributes = configbean.attributes.reduce(function (memo, attr) {
        ...
        if (!attr.isNew) {
            ...
            deletedProperties.push(attr.name);
        }
        return memo;
    }, []);
    ...
    if (deletedProperties.length) {
        configbean.deletedProperties = deletedProperties;
    }
...
function setWatchFile(dirPath, bean) {
    ...
    if (!fileWatchList[dirPath] && bean) {
        ...
        fs.watchFile(dirPath, { persistent: false }, function fileChanged(curr, prev) {
            if (curr.mtime !== prev.mtime && fs.existsSync(dirPath)) {
                if (bean && bean.id) {
                    debug('reloaded: ', bean.id, ' group: ', bean.group, ' managed: ', dirPath, ' of: ', bean);
                    const lastUpdated = getLastUpdated(bean);
                    bean = loadBeanData(bean.id);
                    const currentTimestamp = getLastUpdated(bean);

                    var emitBean = savedConfigBeans[bean.id];
                    if (emitBean && (lastUpdated !== currentTimestamp || emitBean.configbean.deletedProperties)) {
                        emitBean.emit('changed', emitBean.configbean.deletedProperties);
                        delete emitBean.configbean.deletedProperties;
                    }
                }
            }
        });
