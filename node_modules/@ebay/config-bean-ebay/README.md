...
supports a get, set and clear method (which deletes all attributes). The bean also
...
```javascript
var cfgBean = require('@ebay/config-bean-ebay').getBeanById(beanId);
```
...
## Reset a config bean programmatically
...
## Reset a config bean to default state across VM/application restarts
By default config bean values are persisted across application restarts. If you want to reset the config bean values, to the state at the time of definition, after application/VM restart, you can mark the bean as non-persistent by setting the property `persist:false`. 
```   //Sample non-persisting bean
configBean.define({
    'configbean': true,
    'persist': false,
    'id':'nodejs.config.nonpersist.example',
    'group':'nodejs.config',
    'attributes':[{
        'name': 'key',
        'value': 'value',
