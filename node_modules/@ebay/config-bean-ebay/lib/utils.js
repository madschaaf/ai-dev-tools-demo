...
function isDeployRestart() {
    let status = false;
    let deployLogPath = path.join(process.cwd(), 'logs/deploy.log');
    if (fs.existsSync(deployLogPath)) {
        let fileStat = fs.statSync(deployLogPath);
        let deployTime = new Date(fileStat.mtime);
        let diffInSecs = (Date.now() - deployTime) / 1000;
