...
[![Build Status](https://go/-/nodejsci/job/nodejs/job/service-client-ebay/job/master/badge/icon)](https://go/-/nodejsci/job/nodejs/job/service-client-ebay/job/master/) [![Dependency Status](https://appvalidator.vip.qa.ebay.com/nodejs/service-client-ebay.svg)](https://appvalidator.vip.qa.ebay.com/nodejs/service-client-ebay)
[![Code Coverage](https://badgeme.vip.qa.ebay.com/badge/sonar/v0/coverage/service-client-ebay)](http://sonar/dashboard/index?id=service-client-ebay)
...
For more example, please check @ebay/service-instruments-ebay module [reamdme](https://github.corp.ebay.com/nodejs/service-instruments-ebay#service-configuration)
...
Code:
```javascript
// mark the ownership of the client config
const Provider = require('@ebay/service-instruments-ebay').use(__dirname);
// you can share this between requests
const sharedClient = await Provider.createClient('my-graphql-client');
// create a client mapped to the web request
const requestBoundClient = sharedClient.webContext(webRequest);
// run request
const response = await requestBoundClient.query('query operationName { data }');
```

#### File upload with GrpahQL
...
const requestBoundClient = sharedClient.webContext(webRequest);
// run request
const response = await requestBoundClient.query(`mutation($file: Upload!) {
         singleUpload(file: $file) {
              id
         }
     }`,
     {
         file: Buffer.from('some data in buffer')
     });
```

### Browser side

The direct communication of browser and service is not possible because the security token is generated based on application configured as Ajax/JavaScript which is impossible as the frontend host application is usually already configured as Hosted (to allow FE to services communications). 

One solution to this issue would be a creation of application alias that would be configured as secondary app that can be configured as Ajax/JavaScript.

The current solution is to route service calls through host FE application as browser => FE => service.
This option has pros and cons.

Pros:

* Avoid CORS requirements that turned out to be a major pain for the developers.
* Prevent exposure of security tokens on browser side and refresh issues related to that.
* All context is bound to cookies context.

Cons:

* Extra hop/latency between browser and service

#### Enable service on browser side

The special `service-use` tag helps to export your service configuration from config.json file to the browser side. 
...
<service-use id='my-service-with-oauth' />
```
#### Service configuration declaration for ajax bridge
```json
{
    "service-client-ebay": {
        "bridge": true
    ...
    "services": {
        "my-service-with-oauth": {
            ...
            "socketTimeout": 1500,
            "basepath": "/itm",
            "bridgePath": "ajax-service-bridge", // <<< this is default and should be used only if you need to modify the path to a different route
            "oauth": {
                "token": ["user"],
                "scope": "http://api.ebay.com/oauth/scope/@public"
            },
...
#### Service bridge configuration in routes.json

Now that the service call has to go through FE applciation, one needs to configure a route for all bridge calls as follow:

routes.json:
```json
{
    "path": "* /ajax-service-bridge",
    "handler": "require:@ebay/service-client-ebay/bridge",
    "pageName": "bridge",
    "allowedServices": [
        "my-service-with-oauth"
    ]
...
Note: to prevent exploiting this feature to call other services via browser framework requires you to list explicitly in allowedServices which service configs are allowed to be used via bridge.

#### Service endpoint resolution using service-discovery
`discovery` is a default instrument which enables serviceEndpoint discovery for a given serviceId through [service-discovery-ebay](https://github.corp.ebay.com/nodejs/service-discovery-ebay/). See this [example](https://github.corp.ebay.com/nisurana/service-instruments-ebay/blob/master/README.md#example-of-service-with-service-discovery-disabled) on how to disable discovery for a service.
If a service urn is not found in service-discovery, then it will fallback to the host defined in `config.json`. See sample configuration and code below
```json
{
    "services": {
        "my-service-to-discover": {
            "urn": "urn:REST:seskeysvc",        // service-discovery resolution key
            "hostname": "api.ebay.com",     // fallback endpoint
            "basepath": "/itm",
            "applicationLabels": {
                "key1": "value1"
            }
...
```
```javascript
var Provider = require('@ebay/service-client-ebay'); // make sure you have service-client-ebay as a dependency
var client = Provider.context(req).getClient('my-service-to-discover');     //endpoint is dynamically searched using service-discovery-ebay 
client.get({
    foo: 'bar'
...
#### Calling service
```javascript
var Provider = require('@ebay/service-client-ebay') // make sure you have service-client-ebay as a dependency (browser.json)
...
Usually frontend app hosted, say, at www.ebay.com needs to make a service call to experience service or other service hosted at api.ebay.com that by CORS standards is a different domain and the calls from different domains should be marked as allowed by the service via Access-Control-Allow-Origin response header.

One also needs to understand the life cycle of CORS request:

* Initiation of a service call from browser
  * Initiate OPTIONS call that will be validated by the service and either denied or allowed
     * if allowed, the app can dynamically or staticaly state the allowed domains, ex.: Access-Control-Allow-Origin: www.ebay.com, www.ebay.de, etc.
     * some developers try to use *.ebay.com to allow multiple domains, but this does not work, as the value should be explicit or just *. Note, the use of * may cause security issues. Usually it is used for local development or in staging.
     * Also common mistake at server side is to not even implement OPTIONS method; which automatically results in access denied response.
     * Any headers from service client that are not allowed via Access-Control-Allow-Headers and methods not allowed via Access-Control-Allow-Methods will result in service call being denied.
     * Service client can set some headers that are not allowed and not important to the service. In this case, one can filter them our on client side by using `allow-headers` attributes in service configuration.   
  * Initiate a real service call

Note: another common mistake when manipulating one's service config is forgetting deleting .beans folder that preserves service configuration during re-starts.
...
One can add multiple types (string, buffer, stream) to the request body when using multipart/form-data content-type
...
const request = {
    method: 'POST',
    body: {
        foo: fs.createReadStream('./test/fixtures/file.txt'), // read streams
...
Provider.context(req|context).getCLient('multipart-client-id').request(request, callback);
```
Configuration:
```json
{
...
In case you want to see full structure of your request for a specific hostname in dev mode, you can set 
`export DEBUG_REQUEST_HOSTNAME={hostname}`

Foe example:

```bash
export DEBUG_REQUEST_HOSTNAME=svc1.qa.ebay.com
```
...
```json
{
    "instruments-metadata": {
        "cal": {
            "module": "@ebay/service-instruments-ebay/cal",
            "priority": 10
        },
        "cache": {
            "module": "@ebay/service-instruments-ebay/cache",
            "priority": 15
        }
    }
}
...
```javascript
var service = require('@ebay/service-client-ebay');
...
var Adaptor = require('@ebay/service-client-ebay/adaptor').Adaptor;
...
```javascript
// get client
var client = require('@ebay/service-client-ebay').context(req).getClient('my-service');
client.get({
    param1: 'value1',
    param2: 'value2',
})
...
```javascript
var serviceClient = require('@ebay/service-client-ebay');
...
```javascript
require('@ebay/service-client-ebay').context(req).getClient('soap-resource-persistence-service')
    .post({
        body: {
...
```
## Other examples
Find more transport examples [here](https://github.corp.ebay.com/nodejs/service-instruments-ebay#service-configuration).
