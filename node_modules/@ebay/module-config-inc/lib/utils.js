...
function mergeWithNconf(output) {
    ...
    // we need check nconf.get to avoid issues when jest is used
    // in some cases it would not instrument it correctly due to lazy loading
    if (nconf && nconf.get && nconf.get()) {
        outputTemp = {};
...
function generateCacheKey(modRoot) {
    var pkg = getPackage(modRoot);
    return pkg && pkg.name && pkg.name.split('/').pop() || modRoot.split(path.sep).pop();
...
function loadModuleConfig(options, callback) {
    ...
    if (asyncMode) {
        const deferredCacheKey = `__deferred__${cacheKey}`;
        let deferredConfig  = cache.get(deferredCacheKey);
        if (!deferredConfig) {
            deferredConfig = new Promise((resolve, reject) => {
                loadConfig((err, config) => {
                    cache.del(deferredCacheKey);
                    if (err) {
                        return reject(err);
                    }
                    resolve(config);
            ...
            cache.put(deferredCacheKey, deferredConfig);
        }
            ...
            function loadAppConfig(defaultConfig, next) {
                ...
                confiter.create(function (err, configObject) {
                    if (cache.get(cacheKey)) {
                        return next(null, cache.get(cacheKey));
                    }
                    ...
                    // vvvvv----- Disabling this as we now cache via promise
                    ...
                    // enhance configObject/confit with remove entry function
                    configObject.remove = configObject.remove || function remove(key) {
...
function cleanShellOutputForDebug(str) {
    // Remove everthing after the last "}"
    // This is useful in dev mode where the debugger sometimes adds: 
    // "Waiting for the debugger to disconnect..." (or similar) at the end of shell.
    return str.replace(/\}[^\}]+$/, '}');
}
...
module.exports = {
    ...
    cleanShellOutputForDebug: cleanShellOutputForDebug
};
