...
var cache = require('memory-cache'),
    ...
    RaptorConfig = require('@ebay/raptor-config-ebay'),
...
function onConfigBeanChange(cacheKey, bean, emitEvent, mappings, deletedProps) {
    ...
    if (configObject && deletedProps) {
        // just in case memcache gets object from older version, then we use old method 'set'
        const op = configObject.remove ? 'remove' : 'set';
        deletedProps.map(name => configObject[op](name.replace(/___/g, ':')));
    }
    ...
    if (configObject && beanValues) {
        for (index in beanValues) {
            ...
            //When a new property is added to configbean, it's mapping is not there
            if (mappings && !mappings[configObjectKey] && configObjectKey !== 'SVC_CHANNEL_STATUS' && configObjectKey.indexOf(":") === -1) {
                let prefix = findPrefix(mappings);
                configObjectKey = prefix + configObjectKey;
            } else {
    ...
    if (configObject && (hasChanged && emitEvent || deletedProps?.length)) {
        configObject.emit('change');
    }
}

/** Return the prefix used by most of the mappings **/
function findPrefix(mappings) {
    let frequencyMap = {};
    let maxFreq = 0;
    let maxFreqPrefix;
    for (let key in mappings) {
        let prefix = mappings[key].replace(key, '');
        frequencyMap[prefix] = frequencyMap[prefix] ? frequencyMap[prefix] + 1 : 1;
        if (frequencyMap[prefix] > maxFreq) {
            maxFreq = frequencyMap[prefix];
            maxFreqPrefix = prefix;
        }
    }
    return maxFreqPrefix;
}
...
function registerRaptorConfigs(name, configObject, beans) {
    ...
    if (raptorConfigDef && beans) {
        for (var index in raptorConfigDef) {
            var prop = JSON.parse(JSON.stringify(raptorConfigDef[index])),
                keyPath = null;
