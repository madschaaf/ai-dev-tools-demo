...
Modules or app can simply use `@ebay/module-config-inc` for reading the configurations.
...
modConfig(__dirname, function(err, config) {
...
**Note:** 
For a long time at eBay the code has been passing `module` as the first parameter. This may not work with `webpack`. Passing `__dirname` as the first parameter is a better alternative that better supports `webpack`.

Passing `module` or `__dirname` as the first parameter works fine as long as you have no other `package.json` files except at the root
of your module. If there is a `package.json` file on the path from where you run module config up to the root, you should pass the path to the module root as a string when setting up module config.
...
**NOTE**: Be sure that your raptorconfig is **not** created with an MDDF resource or format when you initially create it, or it will not work. Module-config-inc only supports TSV/MongoDB configuration files. 
...
```javascript
{
    "services": {
        "servicename": {
            "urn":"urn:REST:serviceId",             //(optional) is used for service-discovery
            "protocol": "http:",
...
```
NOTE: service urn if exists, can be found [here.](http://raptor/raptor/2.9.x/docs/Service-Discovery/service-registry.md)
...
**In module's config.json `moduleName` should be same as the module folder otherwise config won't be created. e.g  In `@ebay/ratelimiter-ebay` module, use `@ebay/ratelimiter-ebay` in `@ConfigBean` & `@RaptorConfig`**
...
Editing & display of raptor configuration of type `properties` [is not supported](https://jirap.corp.ebay.com/browse/NODE-1704). Instead, such properties needs to be flattened within raptor config as primitives (string, boolean etc.)
...
function makeUrlCall(cb) {
      moduleConfig(__dirname, (err, cfg) => {
          if (err) {
             return cb(err);
          }
...
There are a few cases when app needs to know when remote config is loaded. The remote config does not have any special signal, but one can use the following technique to accomplish the given task
...
async function getConfigValue() {
   config = config || await (async () => {
       const initialConfig = await loadConfig(__dirname);
       return new Promise((resolve, reject) => {
           // we need to handle the case when remote config has been loaded before before app or worker restart
           if (initialConfig.get('key that will always be there') !== undefined) {
               // given the config is already loaded, just return it.
               resolve(initialConfig);
               return;
           }
           // timer is needed in case something went wrong and app eventually needs to timeout
           const timer = setTimeout(() => reject(new Error('Timeout loading remote config')), 50000);
           initialConfig.once('change', () => {
               // clear the timeout timer as we got our remote config
               clearTimeout(timer);
               resolve(initialConfig);
           });
       })
