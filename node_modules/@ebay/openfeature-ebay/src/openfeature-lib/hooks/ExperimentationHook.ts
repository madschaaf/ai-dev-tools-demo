...
import { Details, FlagEvalDetail } from '../types/FlagEvalDetail';
...
export class ExperimentationHook implements Hook {
    context: Context;
    ...
    constructor(context) {
        ...
        this.logger = require('@ebay/logging-inc').logger(
            'openfeature-ebay:experimentationHook',
        );

    }

    async before(hookContext: HookContext) {
        ...
        if (hasStoredData) {
            ...
            const shouldRetryEvaluation = contextHasChanged(
                evalContext,
                defaultContext,
            );
            ...
            if (!flagIsPresent) {
                ...
                if (!data || !data.evaluationResultsMap || data.evaluationResultsMap.size === 0) {
                    ...
                    this.context.setEpTagsEventMap(translateTags(data.epTagsEvent));
                }
            } else {
            ...
            const evaluationResults = _evaluationResults as any;

            evalContext[EVALUATION_RESULTS] = evaluationResults;
            evalContext[SHOULD_RETRY_EVALUATION] = contextHasChanged(
                evalContext,
                defaultContext,
            );
        }
    ...
    after(
        ...
        const {
            flagMetadata: { isPureFeatureFlag },
        } = evaluationDetails;

        if (
            hookContext.context.dropTag !== false &&
            !isPureFeatureFlag &&
 ...
 */
export function contextHasChanged(currentContext, defaultContext) {
    ...
    const contextisUnchanged = Object.keys(currentContext.contextParams)
        ...
        .every(
            (key) =>
                currentContext.contextParams[key] ===
 ...
 * @returns 
 */
export function mergeContextParams(currentContext, defaultContext) {
    return {
        ...currentContext,
        contextParams: {
 ...
 */
export function isEvaluationSatisfied(flagEvalDetail: FlagEvalDetail): boolean {
