...
function _makeRequest(method, options, body, callback) {
    ...
    if (body && Utils.supportsBody(method)) {
        ...
        const isReadable = options.body instanceof Readable;
        options.headers = Utils.normalizeContentType(options.headers);
        const isMultipart = options.headers && options.headers['content-type'] === 'multipart/form-data';
        const bodyCheck = (options.body instanceof Buffer) || (typeof options.body === 'string');
        Assert.ok(((isMultipart || isReadable) && options.body) || 
            bodyCheck, 'Expected a buffer or string for body.');

        if (typeof options.body === 'string') {
            options.body = new Buffer(options.body, 'utf8');
        }
        
        if (!isMultipart && !isReadable) {
