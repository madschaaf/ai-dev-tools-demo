...
describe('marko-request-context', () => {
    ...
    it('render with global platform', () => makeRequest((req, res) => {
        let platform = {
            request: req,
            response: res,
            ebay: req.ebay
        };
        let out = template.render({ $global: { platform } });
        assert(getRequest(out) === platform.request, 'incorrect request');
        assert(getResponse(out) === platform.response, 'incorrect response');
        assert(getEbay(out) === platform.ebay, 'incorrect ebay');
    }));
